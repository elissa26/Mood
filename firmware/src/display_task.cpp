...
void DisplayTask::run() {
    ...
    while(1) {
        ...
        if (num_positions > 1) {
          int32_t height = state.current_position * TFT_HEIGHT / (num_positions-1);
        ...
        if (num_positions > 0) {
          float range_radians = (num_positions - 1) * state.config.position_width_radians;
        ...
        if (num_positions > 0) {
          ...
          } else if (state.current_position == num_positions - 1 && state.sub_position_unit > 0) {
            adjusted_sub_position = logf(1 + state.sub_position_unit  * state.config.position_width_radians / 5 / PI * 180)  * 5 * PI / 180;
          }
        }
